...
X_train, X_test, y_train, y_test = train_test_split(lemmatized_X, y, test_size=0.2, random_state=1)
...
svm_model.fit(X_tfidf_train, y_train)# predict the labels on validation dataset
y_pred_svm = svm_model.predict(X_tfidf_test)# Use accuracy_score function to get the accuracy

# source: https://scikit-learn.org/stable/modules/classes.html#module-sklearn.metrics
# sklearn metric is used to  calculate Accuracy, Precision, Recall and F1 score to evaluate performance
...
# code to display text, actual vs predicted value in tabular format
